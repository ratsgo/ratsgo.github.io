...
def rotate_right(self, node):
   # node의 기존 parent(위 그림에서 B)의 데이터 값을 임시 저장
...
그런데 $V$는 $U$의 왼쪽 자식노드이고, 새 노드는 $V$의 왼쪽 서브트리에 추가됐으므로 *double rotation*을 수행해 줍니다. 여기에서 $W$는 $V$의 자식노드입니다. 다음과 같습니다.
...
위 예시($V$가 $U$의 왼쪽 자식노드, $V$의 오른쪽 서브트리에 새 노드 삽입)를 파이썬 코드로 구현하면 다음과 같습니다.

```python
# node(위 예시에서 W)를 중심으로 double rotation
def double_rotate_leftright(self, node):
	self.rotate_left(node.getLeft().getLeft())
	self.rotate_right(node)
```

$V$가 $U$의 오른쪽 자식노드, $V$의 왼쪽 서브트리에 새 노드를 삽입하는 경우라면 위의 반대로 수행해 줍니다.

```python
def double_rotate_rightleft(self, node):
