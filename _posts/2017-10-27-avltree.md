...
이번 글에서는 **AVL 트리**에 대해 살펴보도록 하겠습니다. 이 글은 고려대 김황남 교수님 강의와 위키피디아를 정리하였음을 먼저 밝힙니다. 파이썬 코드는 [이곳](https://github.com/TheAlgorithms/Python/blob/master/data_structures/AVL/AVL.py)을 참고해 적절하게 수정하였습니다. 그럼 시작하겠습니다.
...
*single rotation*은 *rotation*을 한 차례 수행해 위와 같이 원하는 결과를 얻을 수 있는 경우를 가리킵니다. 삽입 연산의 *single rotation*은 다음 두 가지 경우에 $V$($U$의 자식노드, BF 절대값이 1이하)를 중심으로 실시합니다. ($U$는 BF의 절대값이 2 이상인 노드)
...
*right rotation*의 파이썬 코드는 다음과 같습니다.

```python
# node(위 그림에서 A)를 중심으로 single right rotation
def rotate_right(self, node):
   # node의 기존 parent(위 그림에서 B)의 값을 임시 저장
   aux = node.parent.label
   # node(위 그림에서 A)를 새로운 parent로 지정
   node.parent.label = node.label
   # node의 기존 parent(위 그림에서 B)를 
   # node(위 그림에서 A)의 새로운 right child로 지정
   node.parent.right = Node(aux)
   node.parent.right.height = node.parent.height + 1
   # node의 기존 right child(위 그림에서 베타)를
   # 기존 parent 노드(위 그림에서 B)의 
   # 새로운 left child로 지정
   node.parent.left = node.right
```

*left rotation*은 위의 반대로 수행하면 됩니다.
...
*rotation* 한 차례만으로 원하는 삽입 결과를 내지 못하는 케이스가 존재합니다. 다음 두 가지 경우 $U$를 중심으로 *double rotation*을 수행해 줍니다. ($U$는 BF의 절대값이 2 이상인 노드, $V$는  $U$의 자식노드이면서 BF 절대값이 1이하)
...
그런데 $V$는 $U$의 왼쪽 자식노드이고, 새 노드는 $V$의 왼쪽 서브트리에 추가됐으므로 *double rotation*을 수행해 줍니다. 다음과 같습니다.

- 첫번째 : $V$를 중심으로 *left rotation* 수행 ($A$를 잡아 당겨 내리는 과정)
- 두번째 : $U$를 중심으로 *right rotation* 수행 ($D$를 잡아 당겨 내리는 과정)
...
하지만 서브트리 $W$에 0.8이 추가되면서 $V$와 $U$의 BF는 각각 -1, 2가 됐습니다. $U$를 루트노드로 하는 서브트리가 재구성 대상입니다. 그런데 $V$는 $U$의 왼쪽 자식노드이고, 새 노드(0.8)는 $V$의 오른쪽 서브트리에 추가됐므로 *double rotation*을 수행해 줍니다. $V$를 중심으로 *left rotation*을 수행한 뒤 $U$를 중심으로 *right rotation*을 합니다. 
...
## double rotation의 파이썬 구현

*double rotation*은 *single rotation*을 두 번 수행해 주면 됩니다. 다음과 같습니다.
...
시나리오별로 네 가지 경우의 수가 생깁니다. 다음과 같습니다.
...
위 네 가지 경우의 수를 나눠서 계산해주는 함수는 다음과 같습니다.
