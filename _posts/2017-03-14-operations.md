...
이번 포스팅에서는 **머신러닝**, **데이터마이닝** 기초인 **행렬 연산(Matrix Operations)**에 대해 다뤄 보려고 합니다. **선형대수학(Linear Algebra)**이나 증명 수준이 아니라 연산의 정의 정도를 간단히 다루는 것이니 깊은 내용을 원하시는 분들은 [이곳](http://darkpgmr.tistory.com/103)을 참고 바랍니다. 이번 포스팅은 기본적으로 [고려대 김성범 교수님](http://dmqm.korea.ac.kr/content/page.asp?tID=101&sID=108) 강의를 참고했습니다.
...
위 공분산 행렬의 해석은 이미 설명드린 바와 같습니다. X, Y, Z의 분산은 각각 1.67, 1.67, 4.92입니다. cov(X,Y)=cov(Y,X)=-1.67, cov(Y,Z)=cov(Z,Y)=-1.5,  cov(X,Z)=cov(Z,X)=1.5입니다.
