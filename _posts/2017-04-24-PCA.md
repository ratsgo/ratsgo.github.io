...
\end{align*}
$$
사실 위와 같은 식의 선형결합은 **선형변환**으로도 이해할 수 있습니다. 바꿔 말하면 벡터 $z_i$는 $X$를 $α_i$( p x 1)라는 새로운 축에 **사영(projection)**시킨 결과물이라는 것이죠. 선형변환에 대해 살펴보시려면 [이곳](https://ratsgo.github.io/linear%20algebra/2017/03/24/Ldependence/)을, 사영에 대해 보시려면 [이곳](https://ratsgo.github.io/machine%20learning/2017/03/21/LDA/) 방문을 권해 드립니다. 어쨌든 변수추출로 새롭게 만들어진 $z_i$로 구성된 행렬 $Z$는 아래와 같이 적을 수 있습니다.
...
\end{align*}
$$
**고유벡터(eigenvector)**의 정의에 의해 $α$는 $Σ$의 고유벡터, $λ$는 $Σ$의 고유값이 됩니다. 따라서 원데이터 $X$의 분산을 최대화하는 $α$는 $Σ$의 고유벡터라는 사실을 알 수 있습니다. $Σ$의 고유벡터를 **주성분(Principal Component)**이라고 합니다.

한편 공분산행렬은 **정방행렬(square matrix)**이자 **비특이행렬(non-singular matrix)**로 고유값과 고유벡터의 개수가 차원수($p$)만큼 존재하고, 고유벡터끼리는 서로 **직교(orthogonal)**합니다.
...
\end{align*}
$$
요컨대 $Σ$의 제1 고유벡터로 만든 새로운 변수 $z_1$의 분산은 그에 해당하는 고유값 $λ_1$이라는 뜻입니다. $z_i$의 분산을 모두 합친 값, 고유값 전체 합, 원데이터 $X$의 분산은 서로 같습니다.
...
마찬가지 방식으로 $z_2, z_3$을 만들어 $Z$를 계산할 수 있습니다.
