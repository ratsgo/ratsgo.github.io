...
앞서 말씀드렸듯 $x_0$, $x_1$, $x_2$는 모두 0 이상이기 때문에 $δ$가 양수라면 $w_i$의 그래디언트가 항상 양수, 반대라면 모두 음수 값을 갖게 될 것입니다. 따라서 데이터 $x$와 파라메터 $w$를 2차원 벡터로 가정해 본다면 $w$의 그래디언트는 2사분면과 4사분면 쪽 방향이 될 수는 없습니다.
