...
$Vol(A)$는 $A$에 속한 노드에 연결된 엣지들의 모든 가중치 합입니다. 따라서 $Vol(A)$는 $Cut(A,A)$의 두 배에 $Cut(A,B)$를 더해준 값과 같습니다. 이는 $Vol(B)$도 마찬가지입니다.
...
이해를 돕기 위해 예를 들어보겠습니다. 아래와 같이 무방향 가중치 그래프가 주어졌고, 붉은색 선을 기준으로 부그래프가 두 개로 나뉜다고 합시다. 노드1와 노드2를 $A$, 노드3와 노드4를 $B$로 두겠습니다.
...
최종적으로 계산한 결과는 아래와 같습니다. 여기에서 $w_{14}$와 $w_{23}$은 원그래프에서 두 개 부그래프로 나눠질 때 끊어지는 가중치들입니다. 다시 말해 목적함수에 정의된 $1/4q^T(D-W)q$를 최소화한다는 것은 끊어지는 가중치들을 최소로 하겠다는 의미가 되는 것입니다.
...
위 목적함수를 최소화하는 데 제약조건은 벡터 $q$의 각 요소를 제곱해 모두 더한 $q^Tq$가 전체 노드수와 일치해야 한다는 겁니다. 벡터 $q$의 각 요소는 해당 노드가 $A$에 속해 있으면 1, $B$에 속해 있으면 -1의 값을 가지기 때문입니다. 

이를 라그랑지안 문제로 변환하고, 식을 미지수 $q$로 미분해서 0으로 놓으면 아래와 같이 정리할 수 있습니다. ($q$가 미지수가 되는 이유는 각 노드가 $A$에 속하는지, $B$가 속하는지 정보가 이 벡터에 들어있기 때문입니다)
...
최종 도출된 식을 보면 우리가 찾고자 하는 $q$는 $D-W$ 행렬의 **고유벡터(eigenvector)**임을 알 수 있습니다. 다만 최소값을 찾는 것이 목적이므로 두번째로 작은 고유벡터를 취합니다(제일 작은 고유값은 0이기 때문에 무의미한 해입니다).



## Minimum Cut method 적용 예시

원데이터로부터 인접행렬 $W$를 구축한 뒤 이로부터 $D-W$를 구합니다. 6개 레코드(노드)로 이뤄진 원데이터로부터 도출된 행렬을 고유분해한 결과가 아래와 같다고 치겠습니다. $Λ$는 고유값들로 이뤄진 벡터, $X$는 고유벡터들로 구성된 행렬입니다.
...
그렇다면 우리가 찾고자 하는 미지수 $q$는 $X$의 두번째 열이 됩니다. 여기에서 양수인 노드1 노드2 노드3은 부그래프 $A$, 음수인 노드4 노드5 노드6은 $B$에 할당하면 Spectral Clustering 절차가 끝납니다.
