...
자연언어는 단어나 표현의 등장 순서가 중요한 **sequential data**입니다. 아래 예문을 보시죠.
...
![rnn](http://i.imgur.com/MIlIxoJ.png)
...
![CNN sententce classification architecture](http://i.imgur.com/JN72JHW.png)

이 논문은 영화 리뷰 사이트에 게시된 댓글과 평점 정보를 이용해 각 리뷰가 긍정인지 부정인지 분류하는 모델을 만들고자 했습니다. n개의 단어로 이뤄진 리뷰 문장을 각 단어별로 k차원의 행벡터로 임베딩합니다. 위 그림을 보면 CNN 필터의 크기는 2와 3인데요, 각각 Bigram, Trigram 모델을 뜻한다고 볼 수 있겠습니다. 이후 필터 개수만큼의 **feature map**을 만들고, **Max-pooling** 과정을 거쳐 클래스 개수(긍정 혹은 부정 : 2개)만큼의 스코어를 출력하는 네트워크 구조입니다.
...
우선 CNN 문장분류 아키텍처의 입력값과 출력값을 만들어야 합니다. 입력 데이터는 위 텍스트 문장을 숫자들(단어의 ID)의 나열로, 출력 데이터는 긍정/부정 두 범주로 변환하는 작업입니다. 그런데 단어들이 너무 많으면 메모리 문제로 학습 자체가 불가능해지므로 전체 단어 숫자를 좀 줄일 필요가 있습니다. 이를 위해 문장을 띄어쓰기 기준으로 어절로 나누고 어절 처음 두 글자만 잘라 단어 ID 사전을 만들었습니다. 바꿔 말하면 아래 단어들을 모두 한 단어로 보는 방식입니다. 이 방식을 적용하면 전체 단어수**vocab_size**를 아무런 처리를 하지 않았던 기존 대비 4분의 1로 줄일 수 있습니다.
...
지금까지 설명한 방식을 적용해 입력값과 출력값을 만든 결과의 예시는 아래와 같습니다. 왓챠 평점은 5점 만점인데 2.5점보다 높으면 긍정([1,0])을, 2.5점보다 낮으면 부정([0,1]) 범주를 부여했습니다.

```
고마워요 알프레도, 4.0                  [[0, 1, ......], [1,0]]
영화가 주는 감동이란..., 4.5            [[2, 3, 4, ...], [1,0]]
뭘 얘기하고 싶은건데?, 2.0              [[5, 6, 7, ...], [0,1]]
```

텍스트에서 숫자로 바꾼 입력값은 아래 그림과 같이 또다시 변환됩니다. 아래 그림 기준으로 설명드리면 위 박스의 숫자로 바꾼 입력값은 빨간색 박스에 대응됩니다. 문장 길이**sequence_length**에 해당하는 개수만큼의 단어 ID들의 나열이죠. 그리고 (전체 단어수**vocab_size** X 사용자가 지정한 차원수**embedding_size**) 크기의 Lookup 테이블(파란색 박스)을 만듭니다. 파란색 박스의 행벡터들은 개별 단어의 벡터라고 보시면 되는데요, 초기값은 랜덤하게 설정하고요, 이후 학습 과정에서 문장 분류를 가장 잘하는 방식으로 조금씩 업데이트(변화)되게 됩니다. 이렇게 만들어놓은 Lookup 테이블에서 단어 ID에 해당하는 행벡터를 참조(lookup)해서 가져오는 형식으로 입력값 변환을 마무리하게 됩니다. 결과적으로 리뷰 원문으로부터 단어 ID로 바꾼 빨간색 박스와 파란색 Lookup 테이블을 활용해 보라색 박스를 만들어내는 셈이지요. 이렇게 만든 보라색 박스는 위에서 텍스트 CNN의 필터가 작동하는 방식을 설명드린 움짤과 일치합니다.

![cnn input](http://i.imgur.com/EXiUlme.png)

배치 단위로 CNN을 학습할 경우 위 보라색 박스의 차원수는 [batch_size, sequence_length, embedding_size, channel_size]입니다. 그런데 제가 실험한 CNN 구조에선 채널수를 1로 설정하여서 결과적으로 [batch_size, sequence_length, embedding_size, 1]이 됩니다.
...
자, 그러면 텐서플로우 구현의 핵심인 conv layer를 만드는 부분을 볼까요?
...
여기서 embedded_chars_expanded와 W는 각각 입력값과 필터입니다. 입력값에 대해 필터가 슬라이딩하면서 보되, 그 보폭(strides)은 [1, 1, 1, 1]로 하고 엣지 패딩없이 문장을 슬라이딩하라는 지시입니다. 그럼 [1, 1, 1, 1]의 의미는 무엇일까요? 텐서플로우 문서를 보면 [batch_size, input_height, input_width, input_channels]로 정의돼 있습니다. 그러면 보폭이 [1, 1, 1, 1]이라는 건 배치데이터 하나씩, 단어 하나씩 슬라이딩하면서 보라는 의미겠네요. 왜냐하면 우리는 입력값의 너비와 필터의 너비를 embedding_size로 같게 설정했고, 채널수는 1로 모두 고정했기 때문입니다. 바꿔 말하면 보폭 [1, 1, 1, 1] 중 뒤의 두 개는 입력데이터와 필터가 같은 값을 지니기 때문에 슬라이딩할 수 있는 여유 공간이 없다는 뜻이기도 합니다. 

이렇게 conv를 적용한 뒤 의 텐서 차원수는 [batch_size, sequence_length - filter_size + 1, 1, num_filters]가 됩니다. 첫번째 요소 batch_size가 나오는 건 직관적으로 이해 가능하실 것 같고요, 입력 데이터의 높이**sequence_length**와 필터의 높이**filter_size**를 고려하면 두번째 요소, 입력 데이터의 너비와 필터의 너비가 **embedding_size**로 같다는 점을 감안하면 세번째 요소도 이해가 가능할 겁니다. 또 conv layer를 통과한 데이터는 필터 개수**num_filters**만큼에 해당하는 activation map이 만들어진다는 점에서 마지막 요소를 이해할 수 있습니다. 이후 ReLU를 써서 비선형성(non-linearity)을 확보합니다.

이제 Max-pooling하는 부분을 보겠습니다. 여기서 핵심은 **ksize**인데요, Max-pooling하는 영역의 크기가 되겠습니다. 예컨대 ksize가 [1, 2, 2, 1]이라면 배치데이터/채널별로 가로, 세로 두 칸씩 움직이면서 Max-pooling하라는 지시와 같습니다. 아래와 같이 ksize가 [1, sequence_length - filter_size + 1, 1, 1]이고, 직전 과정에서 conv를 적용한 텐서의 차원수가 [batch_size, sequence_length - filter_size + 1, 1, num_filters]임을 고려하면 Max-pooling이 적용된 후의 텐서 차원수는 [batch_size, 1, 1, num_filters]가 됩니다. 
...
이후엔 Max-pooling한 결과물을 합치고 Full-connected layer를 통과시켜 각 클래스에 해당하는 스코어를 낸 뒤 크로스엔트로피 오차를 구하고 Back-propagation을 수행해 파라메터들을 업데이트하는 일반적인 과정을 거칩니다. 
