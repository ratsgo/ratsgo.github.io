...
$$
L\left( X,y,\beta  \right) =\sum _{ i=1 }^{ n }{ \max { \left[ 0,1-{ y }_{ i }\left( { \beta  }_{ 0 }+{ \beta  }_{ 1 }{ x }_{ 1 }+...+{ \beta  }_{ p }{ x }_{ p } \right)  \right]  }  } 
$$
...
$$
L\left( X,y,\beta  \right) =-\sum _{ i=1 }^{ n }{ { y }_{ i }\left( { \beta  }_{ 0 }+{ \beta  }_{ 1 }{ x }_{ 1 }+...+{ \beta  }_{ p }{ x }_{ p } \right)  } 
$$


힌지 로스의 식을 살펴보면 $y_i(β_0+β_1x_1+…+β_px_p)≥1$을 만족하는 데이터의 손실은 무시(=0)합니다. 하지만 로지스틱 회귀에서는 이를 만족하더라도 손실이 0에 가까워지기는 하지만, 완전히 0이 되지는 않습니다. 이를 나타낸 그림은 아래와 같습니다.
