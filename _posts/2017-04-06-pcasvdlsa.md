...
여기서 대각성분이 행렬 M의 특이값인 Σ에 주의할 필요가 있습니다. Σ는 **대각행렬(diagonal matrix)**인데요, 대각행렬의 거듭제곱은 대각원소들만 거듭제곱을 해주면 됩니다. 따라서 Σ의 제곱은 각 대각원소, 즉 행렬 M의 특이값들을 제곱해준 값과 똑같습니다. 그런데 행렬 M의 특이값은 같은 행렬의 고유값에 제곱근을 취한 값과 동일하므로, Σ을 제곱한 행렬은 행렬 M * M의 고유값으로 이뤄진 행렬 Λ입니다. 이는 정확히 주성분 분석의 결과와 같습니다.
